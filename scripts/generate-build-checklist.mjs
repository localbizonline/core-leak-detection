...
// Post-build QA from BUILD-LOG.md — count unresolved errors (errors minus fixes)
const buildLogContent = readText('BUILD-LOG.md') || '';
const qaPassedMatch = buildLogContent.match(/QA.*PASSED|Post-Build.*PASSED/i);
const totalErrors = (buildLogContent.match(/❌ ERROR/g) || []).length;
const totalFixes = (buildLogContent.match(/🔧 FIX/g) || []).length;
const unresolvedErrors = Math.max(0, totalErrors - totalFixes);

if (qaPassedMatch && unresolvedErrors === 0) {
  pass('Build', `Post-build QA passed (${totalErrors} errors logged, ${totalFixes} fixed, 0 unresolved)`);
} else if (unresolvedErrors > 0) {
  fail('Build', `${unresolvedErrors} unresolved errors in BUILD-LOG.md (${totalErrors} total, ${totalFixes} fixed)`);
} else if (totalErrors > 0 && unresolvedErrors === 0) {
  pass('Build', `All ${totalErrors} errors resolved (${totalFixes} fixes applied)`);
} else {
