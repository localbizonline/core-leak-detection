...
/**
 * Derive SA province from city name. Covers major metros and common cities.
 */
function deriveRegion(city) {
  const c = (city || '').toLowerCase();
  const map = {
    'western cape': ['cape town', 'stellenbosch', 'paarl', 'george', 'knysna', 'mossel bay', 'worcester', 'franschhoek', 'hermanus', 'malmesbury', 'saldanha', 'langebaan'],
    'gauteng': ['johannesburg', 'pretoria', 'sandton', 'centurion', 'midrand', 'randburg', 'roodepoort', 'soweto', 'benoni', 'germiston', 'boksburg', 'kempton park', 'alberton', 'edenvale', 'bedfordview', 'fourways'],
    'kwazulu-natal': ['durban', 'pietermaritzburg', 'umhlanga', 'ballito', 'pinetown', 'newcastle', 'richards bay', 'amanzimtoti', 'hillcrest'],
    'eastern cape': ['port elizabeth', 'gqeberha', 'east london', 'makhanda', 'grahamstown', 'uitenhage', 'jeffreys bay'],
    'free state': ['bloemfontein', 'welkom', 'bethlehem', 'kroonstad'],
    'mpumalanga': ['nelspruit', 'mbombela', 'witbank', 'emalahleni', 'secunda', 'white river'],
    'limpopo': ['polokwane', 'tzaneen', 'mokopane', 'louis trichardt', 'makhado', 'thohoyandou'],
    'north west': ['rustenburg', 'potchefstroom', 'klerksdorp', 'mahikeng', 'brits'],
    'northern cape': ['kimberley', 'upington', 'springbok'],
  };
  for (const [province, cities] of Object.entries(map)) {
    if (cities.some(name => c.includes(name))) {
      return province.split(' ').map(w => w[0].toUpperCase() + w.slice(1)).join(' ');
    }
  }
  return 'Gauteng'; // fallback for unknown cities
}
...
function mapClientData(data) {
  ...
  const region = address.region || data.region || deriveRegion(city);
...
function applyIdentityAndContact(configContent, mapped) {
  ...
  const keyValuePairs = [
    // Identity
    ['name', mapped.name],
    ['tagline', `Professional ${mapped.niche} in ${mapped.city}`],
    ['description', `Professional ${mapped.niche.toLowerCase()} in ${mapped.city} and surrounding areas. Quality workmanship, fair pricing.`],
...
function applyDesignTokens(configContent, designData) {
  ...
  // Font replacements — fonts may be strings or objects with .family
  const fonts = designData.fonts || {};
  const displayFont = typeof fonts.display === 'object' ? fonts.display?.family : fonts.display;
  const bodyFont = typeof fonts.body === 'object' ? fonts.body?.family : fonts.body;
  const accentFont = typeof fonts.accent === 'object' ? fonts.accent?.family : fonts.accent;
  if (displayFont) {
    const replaced = content.replace(/displayFont:\s*["'][^"']+["']/, `displayFont: "${displayFont}"`);
    if (replaced !== content) { content = replaced; count++; }
  }
  if (bodyFont) {
    const replaced = content.replace(/bodyFont:\s*["'][^"']+["']/, `bodyFont: "${bodyFont}"`);
    if (replaced !== content) { content = replaced; count++; }
  }
  if (accentFont) {
    // Add accentFont if not already present, or update if it exists
    if (content.includes('accentFont:')) {
      const replaced = content.replace(/accentFont:\s*["'][^"']+["']/, `accentFont: "${accentFont}"`);
      if (replaced !== content) { content = replaced; count++; }
    } else {
...
const FEATURE_BADGE_MAP = {
  'Free Quotes': {
    ...
    label: 'No-Obligation Estimates',
    heroBadge: 'No-Obligation Estimates',
  },
