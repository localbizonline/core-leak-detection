...
// Contrast check entry — one per page per viewport the reviewer actually looked at.
// validate-qa.mjs cross-references these against the screenshot manifest to make sure
// every page was visually inspected (not just HTML-grepped).
const ContrastCheck = z.object({
  page: z.string(),             // route, e.g. "/"
  viewport: z.enum(['desktop', 'mobile', 'tablet']),
  screenshot: z.string(),       // path to the screenshot that was reviewed
  textReadable: z.boolean(),    // overall verdict: is ALL text legible?
  issues: z.array(z.string()).default([]),  // e.g. "footer white text on yellow bg"
});
...
export const DesignReviewResult = z.object({
  ...
  passed: z.boolean(),
  // How many screenshot images the reviewer actually opened & inspected.
  // validate-qa.mjs will REJECT if this is less than the number of successful
  // screenshots in the manifest — proves the agent didn't skip visual review.
  screenshotsReviewed: z.number().int().min(1),
  // Per-page, per-viewport contrast/readability verdicts.
  // Must have at least one entry per page that was screenshotted.
  contrastChecks: z.array(ContrastCheck).min(1),
