...
export function validateQaResults(projectPath) {
  ...
  // Cross-check: design-review.json must prove screenshots were visually inspected
  if (agentData['design-reviewer'] && screenshots.valid && screenshots.captured > 0) {
    const dr = agentData['design-reviewer'];

    // 1. screenshotsReviewed must be >= successful screenshots in manifest
    if (typeof dr.screenshotsReviewed !== 'number' || dr.screenshotsReviewed < screenshots.captured) {
      allErrors.push(
        `design-review.json screenshotsReviewed (${dr.screenshotsReviewed ?? 'missing'}) ` +
        `< manifest screenshots (${screenshots.captured}) — agent did not visually review all pages`
      );
      console.log(`  ✗ Visual review coverage: INCOMPLETE (${dr.screenshotsReviewed ?? 0}/${screenshots.captured})`);
    } else {
      console.log(`  ✓ Visual review coverage: ${dr.screenshotsReviewed}/${screenshots.captured} screenshots reviewed`);
    }

    // 2. contrastChecks must cover every page in the manifest
    const manifestRoutes = new Set();
    const manifestPath2 = path.join(projectPath, 'qa-screenshots', 'manifest.json');
    try {
      const mRaw = JSON.parse(fs.readFileSync(manifestPath2, 'utf-8'));
      for (const s of mRaw.screenshots || []) {
        if (s.status === 'ok') manifestRoutes.add(s.route);
      }
    } catch {} // already validated above

    const checkedRoutes = new Set((dr.contrastChecks || []).map(c => c.page));
    const uncheckedRoutes = [...manifestRoutes].filter(r => !checkedRoutes.has(r));

    if (uncheckedRoutes.length > 0) {
      allErrors.push(
        `design-review.json contrastChecks missing for routes: ${uncheckedRoutes.join(', ')} — ` +
        `text readability was NOT verified on these pages`
      );
      console.log(`  ✗ Contrast checks: missing for ${uncheckedRoutes.length} page(s)`);
    } else {
      console.log(`  ✓ Contrast checks: all ${manifestRoutes.size} pages have readability verdicts`);
    }
  }
