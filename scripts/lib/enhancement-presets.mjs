...
.hero-form-card:focus-within {
  ...
  'split-layout': {
    name: 'Split Hero Layout (Two-Column)',
    description: 'Two-column layout — headline + CTAs left, hero image or empty right column. Layout change only — does NOT inject trust badges, stat cards, or review scores (these duplicate TrustBadges/heroBadges already on the page).',
    when: 'Businesses wanting asymmetric hero layout for visual variety',
...
export const COMPONENT_PRESETS = {
  ...
  'value-strip': {
    ...
    existsInTemplate: true,
    when: 'Business has pricing signals, "Low Cost" feature, or no-obligation estimates',
    ...
    dataShape: {
      ...
      example: [
        { label: 'Geyser Replacements', value: 'No-Obligation Estimates', icon: '🔥' },
...
@keyframes pulse-dot {
  ...
  'price-tags': {
    ...
    when: 'Services have pricing data or estimate messaging',
  ...
  'section-divider': {
    ...
    description: 'SVG shape divider between body sections. 6 shape options. NEVER place in Hero — SVG shapes on hero background images look broken.',
    existsInTemplate: true,
    componentFile: 'SectionDivider.astro',
    when: 'Design direction or competitor analysis suggests shaped transitions between body sections',
    placement: 'Between body sections only (e.g., Services→Reviews, Reviews→CTA). NEVER in Hero.',
    aggressiveness: 'low',
...
.text-center h2::after {
  ...
  'scroll-reveal': {
    ...
    description: 'Sections fade in and slide up as they enter viewport. Uses IntersectionObserver. ONLY for tier 3 (premium/lifestyle) niches — trades/contractors do not benefit from this.',
    aggressiveness: 'medium',
    tierRestriction: [3], // Only allowed for premium/lifestyle services
    css: `/* Scroll reveal */
.reveal {
  opacity: 0;
...
export const NICHE_RECOMMENDATIONS = {
  ...
  'pest control': { hero: 'split-layout', components: ['google-badge', 'years-badge', 'value-strip', 'cta-banner', 'section-divider'], css: ['mixed-case-headings', 'warm-surfaces', 'accent-underline-headings'], ctaBanner: 'stats-bar', sectionDivider: 'curve' },
  ...
  painting:     { hero: 'split-layout', components: ['google-badge', 'years-badge', 'value-strip', 'cta-banner', 'section-divider'], css: ['warm-surfaces', 'accent-underline-headings'], ctaBanner: 'split-image', sectionDivider: 'curve' },
  fencing:      { hero: 'split-layout', components: ['google-badge', 'years-badge', 'price-tags', 'cta-banner', 'section-divider'], css: ['warm-surfaces', 'mixed-case-headings', 'card-hover-lift'], ctaBanner: 'stats-bar', sectionDivider: 'zigzag' },
  ...
  landscaping:  { hero: 'split-layout', components: ['google-badge', 'years-badge', 'cta-banner', 'section-divider'], css: ['warm-surfaces', 'scroll-reveal', 'accent-underline-headings'], ctaBanner: 'split-image', sectionDivider: 'wave' }, // landscaping = tier 3, scroll-reveal OK
  ...
  renovation:   { hero: 'split-layout', components: ['google-badge', 'years-badge', 'value-strip', 'cta-banner', 'section-divider'], css: ['warm-surfaces', 'graduated-weights'], ctaBanner: 'split-image', sectionDivider: 'angle' },
  building:     { hero: 'split-layout', components: ['google-badge', 'years-badge', 'price-tags', 'cta-banner', 'section-divider'], css: ['mixed-case-headings', 'graduated-weights', 'card-hover-lift'], ctaBanner: 'stats-bar', sectionDivider: 'angle' },
...
export const FEATURE_ENHANCEMENTS = {
  ...
  'Top Quality': {
    ...
    forceCss: [],
    heroOverride: null,
    notes: 'Agent should emphasize gallery/portfolio section, add before/after if available. Scroll-reveal only for tier 3 (premium/lifestyle) niches.',
  },
};
...
// Filters out tier-restricted CSS presets automatically.
// ---------------------------------------------------------------------------
export function getRecommendations(niche, features = [], nicheTier = null) {
  const lower = niche.toLowerCase();

  // Resolve tier if not provided
  const tier = nicheTier?.tier || (isEmergencyNiche(niche) ? 1 : 2);
  ...
  if (!nicheRec) {
    nicheRec = {
      ...
      components: ['google-badge', 'years-badge', 'cta-banner'],
  ...
  // Filter out CSS presets that have tier restrictions
  const filteredCss = [...cssPresets].filter(presetName => {
    const preset = CSS_PRESETS[presetName];
    if (!preset?.tierRestriction) return true; // no restriction = allowed everywhere
    return preset.tierRestriction.includes(tier);
  });
