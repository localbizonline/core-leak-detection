...
const NEGATIVE_PROMPT = 'text, words, letters, logos, watermark, signature, label, signage, blurry, low quality, cartoon, illustration, painting, drawing, face, portrait, selfie, person looking at camera, HDR, oversaturated, studio lighting, stock photo, staged, model, posed, showroom, display home';

// ---------------------------------------------------------------------------
// Image size presets — nano-banana-pro uses aspect_ratio + resolution params
// ---------------------------------------------------------------------------
// The model does NOT accept custom pixel dimensions (width/height).
// Instead it uses named aspect ratios: 21:9, 16:9, 3:2, 4:3, 5:4, 1:1, etc.
// Resolution: 1K (default), 2K, 4K
// ---------------------------------------------------------------------------

const IMAGE_SIZES = {
  card:    { aspect_ratio: '4:3',  resolution: '2K' },   // service card thumbnails
  hero:    { aspect_ratio: '21:9', resolution: '2K' },   // page hero banners
  content: { aspect_ratio: '3:2',  resolution: '2K' },   // service content images
  og:      { aspect_ratio: '16:9', resolution: '1K' },   // OG social sharing image (closest to 1200x630)
  about:   { aspect_ratio: '4:3',  resolution: '2K' },   // about page image
  square:  { aspect_ratio: '1:1',  resolution: '1K' },   // 1:1 — fallback
};
...
async function generateImage(prompt, outputPath, options = {}) {
  const { aspect_ratio = '4:3', resolution = '2K' } = options;
  const key = getFalKey();

  // Submit to queue — nano-banana-pro uses aspect_ratio + resolution, not image_size
  ...
  for (let i = 0; i < 30; i++) {
    ...
    if (status.status === 'COMPLETED') {
      ...
      if (result.detail?.[0]?.type === 'content_policy_violation') {
        throw new Error(`Content policy violation: prompt was flagged by FAL safety checker`);
      }
  ...
  if (toGenerate.length > 0) {
    ...
    const results = await Promise.allSettled(
      toGenerate.map(async (task) => {
        ...
        console.log(`    Start: ${task.filename} [${task.options?.aspect_ratio || '4:3'} ${task.options?.resolution || '2K'}]`);
        await retry(
