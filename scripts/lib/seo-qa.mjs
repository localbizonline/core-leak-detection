...
/**
 * Extract all routes — prefers page-registry.json (single source of truth),
 * falls back to site.config.ts + filesystem discovery.
 * @param {string} projectPath
 * @returns {string[]}
 */
export function discoverRoutes(projectPath) {
  // --- Strategy 1: page-registry.json (preferred, authoritative) ---
  const registryPath = path.join(projectPath, 'page-registry.json');
  if (fs.existsSync(registryPath)) {
    try {
      const registry = JSON.parse(fs.readFileSync(registryPath, 'utf-8'));
      const routes = new Set();
      // Support both array format [{route: "/"}] and object format {"/": {...}}
      if (Array.isArray(registry)) {
        for (const entry of registry) {
          const route = entry.route || entry.path;
          if (route) routes.add(route.endsWith('/') || route === '/' ? route : `${route}/`);
        }
        ...
        // Object keys are routes
        for (const route of Object.keys(registry)) {
          if (route.startsWith('/')) routes.add(route.endsWith('/') || route === '/' ? route : `${route}/`);
        }
      }
  ...
  // --- Strategy 2: site.config.ts + filesystem discovery (fallback) ---
  ...
  // Area pages — read actual slug from each location JSON file (not filename)
  ...
  if (fs.existsSync(locationsDir)) {
    ...
    for (const file of files) {
      try {
        const locData = JSON.parse(fs.readFileSync(path.join(locationsDir, file), 'utf-8'));
        // Use the slug field from the JSON (the actual route), not the filename
        const slug = locData.slug || file.replace(/\.json$/, '');
        // Area pages are root-level routes (generated by [slug].astro), not /areas/
        routes.add(`/${slug}/`);
      } catch {
        // Fallback to filename-based slug at root level
  ...
  // Common static pages (use /about-us/ not /about/ — matches template)
  for (const p of ['/about-us/', '/contact/', '/services/', '/reviews/', '/privacy-policy/', '/terms-and-conditions/']) {
    routes.add(p);
  }
