...
function generateGlobalCss(theme, projectDir) {
  ...
  // Auto-determine best text color for accent buttons (P4.1 contrast fix)
  const accent = theme.accent || '#D42027';
  const primary = theme.primary || '#1B2A4A';
  const whiteOnAccent = contrastRatio(accent, '#FFFFFF');
  const accentTextColor = whiteOnAccent >= 4.5 ? '#FFFFFF' : primary;

  // Hero overlay and footer BG — always dark for readability.
  // If designer provided explicit overrides, use those. Otherwise, darken the primary
  // to near-black (lightness capped at 15%) so white text is always readable.
  const heroOverlay = theme.heroOverlay || darkenToReadable(primary);
  const footerBg = theme.footerBg || darkenToReadable(primary);
...
/* Accent button text — auto-picks white or dark for WCAG AA contrast */
...
/* Hero gradient overlay fallback — neutral dark scrim, heavier on text side.
   Template uses bg-hero-overlay/80 by default. If the design-enhancer swaps
   to hero-gradient-overlay, this provides the CSS so the overlay never breaks. */
.hero-gradient-overlay {
  background: linear-gradient(
    to right,
    rgba(10, 10, 12, 0.82) 0%,
    rgba(10, 10, 12, 0.65) 45%,
    rgba(10, 10, 12, 0.35) 75%,
    rgba(10, 10, 12, 0.18) 100%
  );
}

/* Hero CTA — call button uses white for maximum visibility on dark overlay */
...
/**
 * Darken a hex color to ensure white text is always readable on it.
 * Preserves the hue/saturation but caps lightness at 15% — produces a
 * near-black tinted version of the color. If the color is already dark
 * enough (contrast ratio >= 7:1 with white), returns it unchanged.
 */
function darkenToReadable(hex) {
  const ratio = contrastRatio(hex, '#FFFFFF');
  if (ratio >= 7) return hex;   // Already dark enough

  const rgb = hexToRgb(hex);
  const hsl = rgbToHsl(rgb.r, rgb.g, rgb.b);

  // Cap lightness at 15% and reduce saturation to avoid garish dark tones
  const darkL = Math.min(hsl.l, 0.15);
  const darkS = Math.min(hsl.s, 0.4);

  // HSL → RGB conversion
  const c = (1 - Math.abs(2 * darkL - 1)) * darkS;
...
function main() {
  ...
  // Dedup guard: if CSS already exists and has uniqueness/enhancer appends,
  // only overwrite the auto-generated portion (before the uniqueness block).
  const existingCss = existsSync(cssPath) ? readFileSync(cssPath, 'utf-8') : '';
  // Match both old and new uniqueness markers for backwards compatibility
  const newMarker = '/* --- UNIQUENESS TWEAKS --- */';
  const oldMarker = '/* ==========';
  const oldMarkerFull = 'UNIQUENESS TWEAKS';
  const markerIndex = existingCss.indexOf(newMarker) !== -1
    ? existingCss.indexOf(newMarker)
    : (existingCss.includes(oldMarkerFull) ? existingCss.indexOf('/* =====') : -1);
  const hasUniquenessAppend = markerIndex !== -1;

  if (hasUniquenessAppend) {
    // Preserve everything after the uniqueness marker
    const appendedCss = existingCss.slice(markerIndex);
    writeFileSync(cssPath, css + '\n' + appendedCss, 'utf-8');
    log.info('generate-theme', 'Theme CSS regenerated — preserved uniqueness/enhancer appends');
    console.log(`\n✓ global.css regenerated (preserved uniqueness appends)`);
  } else {
